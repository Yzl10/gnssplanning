...
let MEO = ["C11", "C12", "C14", "C19", "C20", "C21", "C22", "C23", "C24", "C25", "C26", "C27", "C28",
    "C29", "C30", "C32", "C33", "C34", "C35", "C36", "C37", "C41", "C42", "C43", "C44", "C45", "C46", "C57", "C58"]; //29颗
let IGSO = ["C06", "C07", "C08", "C09", "C10", "C13", "C16", "C31", "C38", "C39", "C40", "C56"];   //12颗
let GLONASS = ["R1","R2","R3","R4","R5","R6","R7","R8","R9","R10","R11","R12","R13","R14","R15","R16",
    "R17","R18","R19","R20","R21","R22","R23","R24",]

let GALILEO = ["E2","E3","E4","E5","E7","E8","E9","E10","E11","E12","E13","E14","E15","E18","E19","E21",
    "E24","E25","E26","E27","E30","E31","E33","E34","E36",]

let GPS = ["G1","G2","G3","G4","G5","G6","G7","G8","G9","G10","G11","G12","G13","G14","G15","G16","G17","G18","G19","G20",
    "G21","G23","G24","G25","G26","G27","G28","G29","G30","G31","G32",]
...
function DrawOrbit(satellites, orbitType) {
    try {
        ...
        document.getElementById("GALILEOOn").checked = true;
        document.getElementById("GLONASSOn").checked = true;
        EnableSatSelect();
        ...
        for (let i = 0; i < satellites.length; i++) {
            ...
            if(satellite.prn.toString()[0]==='R'){
                satMesh.name = satellite.prn;
                // continue;
            }else if(satellite.prn.toString()[0]==='E'){
                satMesh.name = satellite.prn;
            }else if(satellite.prn.toString()[0]==='G'){
                satMesh.name = satellite.prn;
            }else{
            ...
            }else if(GLONASS.includes(satMesh.name)){
                material = new THREE.LineBasicMaterial({
                    color: 0xF0FFFF,
                });
            }else if(GPS.includes(satMesh.name)){
                material = new THREE.LineBasicMaterial({
                    color: 0x0000FF,
                });
            }else if(GALILEO.includes(satMesh.name)) {
                material = new THREE.LineBasicMaterial({
                    color: 0x00FF00,
                });
            }else {
        ...
        if (orbitType === "ECEF") {
            ...
            directionalLight.position.set(-10000 * Math.sin(-initRotateAngle), 10000 * Math.tan(phi),
                -10000 * Math.cos(-initRotateAngle));
            scene.add(ecefSatGroup);
            scene.add(ecefOrbitGroup);
            scene.add(ecefPrnGroup);
        } else if (orbitType === "ECI") {
            earthMesh.rotation.y = earthRotateY + Theta;
            directionalLight.position.set(-10000 * Math.sin(-initRotateAngle + Theta), 10000 * Math.tan(phi),
                -10000 * Math.cos(-initRotateAngle + Theta));
...
function DisableSatSelect(){
    ...
    document.getElementById("GPSOn").disabled = true;
    document.getElementById("GALILEOOn").disabled = true;
    document.getElementById("GLONASSOn").disabled = true;
}
...
function EnableSatSelect(){
    ...
    document.getElementById("GPSOn").disabled = false;
    document.getElementById("GALILEOOn").disabled = false;
    document.getElementById("GLONASSOn").disabled = false;
}
...
function SetTrackColor(r, prn) {
    if (MEO.includes(prn)) {
        r.setAttribute("stroke", "#EE2C2C");
    } else if (GEO.includes(prn)) {
        r.setAttribute("stroke", "#6495ED");
    } else if (IGSO.includes(prn)) {
        r.setAttribute("stroke", "#FF8C00")
    } else if (GLONASS.includes(prn)){
        r.setAttribute("stroke", "#F0FFFF")
    }else if (GPS.includes(prn)){
        r.setAttribute("stroke", "#0000FF")
    }else if (GALILEO.includes(prn)) {
